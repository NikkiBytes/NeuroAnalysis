...
masker = NiftiMasker(mask_img=imag_mask,
                     standardize=True, memory="nilearn_cache", memory_level=)
...
fileout=open("/projects/niblab/bids_projects/Experiments/ChocoData/Nested_CV_scores_milkshake.txt", "a")
fileout.write("Nested CV score: %.4f" % np.mean(nested_cv_scores))
file.close()


print(">>>----> making the plot and image!")
coef = svc.coef_
...
display.savefig('svm_milkshake_all_linear.png')
# Saving the results as a Nifti file may also be important
weight_img.to_filename('/projects/niblab/bids_projects/Experiments/ChocoData/milkshake_svm_all_linear.nii')
